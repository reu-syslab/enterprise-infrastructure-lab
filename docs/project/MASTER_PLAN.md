...
Tasks
Discover FortiGate interfaces, routes, DHCP, DNS, firewall policy and VIPs.
Discover Proxmox VLAN-aware bridge configuration.
Discover VM/LXC network dependencies.
Discover MikroTik bridge VLAN configuration.
Confirm physical trunk direction.
Capture private FortiGate baseline.
Capture MikroTik .rsc export.
Capture MikroTik binary backup.
Capture Proxmox baseline.
Record pre-existing technical debt.
Identify public/private documentation boundary.
Findings
...
Exit gate
Baseline exists outside public Git.
Critical topology is understood.
Rollback source exists.
No unresolved discovery gap blocks first Enterprise Lab VLAN.
...
Tasks
FortiGate
Create server VLAN interface.
Create client VLAN interface.
Assign gateways.
Permit ICMP for validation.
Keep general policies absent until needed.
MikroTik
Add VLAN60 tagged transport between FortiGate and Proxmox trunks.
Add VLAN70 tagged transport between FortiGate and Proxmox trunks.
Do not create unnecessary L3 interfaces.
Proxmox
Confirm existing VLAN-aware bridge supports new VLAN IDs.
Avoid unnecessary host VLAN subinterfaces.
Validation
Disposable VM tested in VLAN60.
VLAN60 gateway reachable.
Disposable VM moved to VLAN70.
VLAN70 gateway reachable.
0% packet loss in both tests.
Exit gate
End-to-end VLAN60 path proven.
End-to-end VLAN70 path proven.
HomeLab existing VLANs remain operational.
...
Monitoring detects stopped service.
Automation collects service status and recent events.
If incident matches approved Class A rule:
start service;
wait;
verify service;
run functional test.
...
HomeLab base connectivity
FortiGate ENT-LAB networking
MikroTik ENT-LAB VLAN transport
Proxmox VM layer
DC01 / DNS
DC02
identity validation
...
read STATUS.md;
read the current phase in this MASTER PLAN;
execute only the next approved slice;
avoid unrelated cleanup.
At the end:

update STATUS.md;
update relevant architecture/runbook documents;
save private operational evidence;
update checkboxes in this MASTER PLAN only when evidence exists;
commit public-safe changes;
set exactly one clear NEXT ACTION.
...
define VM resources;
create DC01;
attach DC01 to VLAN60;
install Windows Server;
configure static networking;
validate gateway and required outbound connectivity;
accept AD DNS namespace;
install AD DS + DNS;
